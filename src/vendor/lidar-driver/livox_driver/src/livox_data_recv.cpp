...
int LidarDriver::ParsePacket(livox_driver::LivoxLidarPacket & packet)
{
  ...
  do {
    ...
    LivoxExtendRawPointToPxyzrtl(point_base, packet.data, lidar_device_.line_num, timegap_over);
  ...
  bool timegap_over)
{
  ...
  const livox_driver::LivoxPointXyzrtl zero_data{};
  const LivoxExtendRawPoint * raw_point =
    timegap_over ? nullptr : reinterpret_cast<const LivoxExtendRawPoint *>(eth_packet->data);
