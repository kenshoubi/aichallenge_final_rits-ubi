...
{
public:
  enum class LivoxDiagStatus {
    kInitialize = 0,    // initialize
    kHandShakeRetry,    // Hand shake retry.
    kTerminate,         // Ctrl-C terminate
    kHeartBeatError,    // Heart beat retry over.
    kRecvBroadcast,     // Receive Broadcast
    kHandShakeOk,       // Hand shake ack
    kStreamRequest,     // Stream Start request
    kStreamRetry,       // Stream Start Nack retry
    kStreamStop,        // Stream Stop request
    kStreamingOk,       // Streaming and Heart beat ack
    kStreamingHbRetry,  // Streaming and Heart beat retry n
    kErrorNone
  };
  ...
  LivoxDiagStatus GetDiagStatus()
  {
    ...
    if (err_status_ == LivoxDiagStatus::kErrorNone) {
      ...
      ret = err_status_;
      err_status_ = LivoxDiagStatus::kErrorNone;
    }
    return ret;
  }

  bool CheckLidarConnect()
  {
    LivoxDiagStatus status = now_status_;
    if (
      status == LivoxDiagStatus::kHeartBeatError || status == LivoxDiagStatus::kHandShakeRetry ||
      status == LivoxDiagStatus::kInitialize || status == LivoxDiagStatus::kTerminate) {
      return false;
    }
    return true;
  }
  int32_t GetHeartBeatRetryCount() { return heart_beat_retry_cnt_; }
...
private:  // Resource, Configuration
  ...
  HwInterface::UdpSocket broadcast_sock_;
  ...
  bool UdpSocketOpen(
    ...
    uint16_t my_port, bool broadcast = false);
...
private:  // Status, Command
  ...
  volatile LivoxDiagStatus now_status_{LivoxDiagStatus::kInitialize};
  volatile LivoxDiagStatus err_status_{LivoxDiagStatus::kErrorNone};
  void SetDiagStatus(LivoxDiagStatus new_status);

  int32_t heart_beat_retry_cnt_{0};
  void UpdateHeartBeatRetryCount(int retry_cnt, int hb_timeout);
