...
void RosDriverWrapper::MainThread()
{
  ...
  if (config_.debug_mode) {
    RCLCPP_INFO(this->get_logger(), "hb_timeout = %d", sensor_config_ex_.sensor_config.hb_timeout);
  }
...
void RosDriverWrapper::InitializeLivoxDiagnostics()
{
  ...
  diagnostics_updater_.setPeriod(0.1);
}

std::string RosDriverWrapper::LivoxConnectionString(LidarDriver::LivoxDiagStatus diag_status)
{
  const std::map<LidarDriver::LivoxDiagStatus, const std::string> livox_diag_status_dict_ = {
    {LidarDriver::LivoxDiagStatus::kInitialize, "Initialize"},
    {LidarDriver::LivoxDiagStatus::kHandShakeRetry, "Disconnect_hs_retry"},
    {LidarDriver::LivoxDiagStatus::kTerminate, "Disconnect_terminate"},
    {LidarDriver::LivoxDiagStatus::kHeartBeatError, "Disconnect_hb_retry_over"},
    {LidarDriver::LivoxDiagStatus::kRecvBroadcast, "Disconnect_broadcast"},
    {LidarDriver::LivoxDiagStatus::kHandShakeOk, "Connect_hs_ack"},
    {LidarDriver::LivoxDiagStatus::kStreamRequest, "Connect_start_request"},
    {LidarDriver::LivoxDiagStatus::kStreamRetry, "Connect_start_nack_retry"},
    {LidarDriver::LivoxDiagStatus::kStreamStop, "Connect_stop_request"},
    {LidarDriver::LivoxDiagStatus::kStreamingOk, "Streaming_hb_ack"},
    {LidarDriver::LivoxDiagStatus::kStreamingHbRetry, "Streaming_hb_retry"},
    {LidarDriver::LivoxDiagStatus::kErrorNone, "Unknown"}};
  std::string str;

  try {
    str = livox_diag_status_dict_.at(diag_status);
    if (config_.debug_mode && diag_status_ != diag_status) {
      diag_status_ = diag_status;
      RCLCPP_INFO(this->get_logger(), "%s %s", __func__, str.c_str());
    }

    if (diag_status == LidarDriver::LivoxDiagStatus::kStreamingHbRetry) {
      int32_t hb_timeout = sensor_config_ex_.sensor_config.hb_timeout;
      if (hb_timeout > 0) {
        int32_t retry_cnt = driver_.GetHeartBeatRetryCount();
        str += "_" + std::to_string(retry_cnt) + "/" + std::to_string(hb_timeout);
      }
    }
  ...
  diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  uint8_t level;
  LidarDriver::LivoxDiagStatus diag_status = driver_.GetDiagStatus();
  switch (diag_status) {
    case LidarDriver::LivoxDiagStatus::kInitialize:
    case LidarDriver::LivoxDiagStatus::kTerminate:
    case LidarDriver::LivoxDiagStatus::kStreamStop:
    case LidarDriver::LivoxDiagStatus::kStreamingOk:
    case LidarDriver::LivoxDiagStatus::kStreamingHbRetry:
      level = DiagStatus::OK;
      break;

    case LidarDriver::LivoxDiagStatus::kHandShakeOk:
    case LidarDriver::LivoxDiagStatus::kStreamRequest:
    case LidarDriver::LivoxDiagStatus::kStreamRetry:
      level = DiagStatus::WARN;
      break;

    default:  // LivoxDiagStatus::kHandShakeRetry or kHeartBeatError or Unexpected error
      level = DiagStatus::ERROR;
      break;
  }

  std::string str = LivoxConnectionString(diag_status);
  diagnostics.add("sensor", config_.frame_id);
  diagnostics.summary(level, str.c_str());
}
  ...
  diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.temperature_status : Temp::kUnknown;
  switch (status) {
    case Temp::kUnknown:
      level = DiagStatus::ERROR;
      break;
...
void RosDriverWrapper::LivoxCheckVoltage(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.voltage_status : Volt::kUnknown;
  switch (status) {
    case Volt::kUnknown:
      level = DiagStatus::ERROR;
      break;
...
void RosDriverWrapper::LivoxCheckMotor(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.motor_status : Motor::kUnknown;
  switch (status) {
    case Motor::kUnknown:
      level = DiagStatus::ERROR;
      break;
  ...
  diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.dirt_status : WindowDirt::kUnknown;
  switch (status) {
...
void RosDriverWrapper::LivoxCheckFirmware(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.firmware_status : Firmware::kUnknown;
  switch (status) {
...
void RosDriverWrapper::LivoxCheckTimeSync(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.time_sync_status : TimeSync::kUnknown;
  switch (status) {
  ...
  diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status = driver_.CheckLidarConnect() ? current_diagnostics_status_.device_life_status
...
void RosDriverWrapper::LivoxCheckFan(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.fan_status : Fan::kUnknown;
  switch (status) {
    case Fan::kUnknown:
      level = DiagStatus::ERROR;
      break;
...
void RosDriverWrapper::LivoxCheckPtp(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.ptp_status : Ptp::kUnknown;
  switch (status) {
    case Ptp::kNoSignal:
    case Ptp::kUnknown:
      level = DiagStatus::WARN;
...
void RosDriverWrapper::LivoxCheckPps(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  auto status =
    driver_.CheckLidarConnect() ? current_diagnostics_status_.pps_status : Pps::kUnknown;
  switch (status) {
    case Pps::kUnknown:
    case Pps::kNoSignal:
      level = DiagStatus::WARN;
